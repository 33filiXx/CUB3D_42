...
void	render_helper(t_sprite *sprite, t_frame *frame, t_game_data *data,
		t_render_sprite *render_s)
{
	while (render_s->y <= sprite->draw.draw_end_y)
	{
		render_s->v = (double)(render_s->y - sprite->draw.draw_start_y)
			/ (double)frame->span_y;
		if (render_s->v < 0.0)
			render_s->v = 0.0;
		if (render_s->v > 1.0)
			render_s->v = 1.0;
		render_s->tex_y = sprite->draw.origin_y + (int)(render_s->v
				* sprite->frame_h);
		...
		render_s->color = texel(sprite->sp_tex, render_s->tex_x,
				render_s->tex_y);
...
void	render_sprites(t_sprite *sprite, t_frame *frame, t_game_data *data)
{
	...
	while (render_s.x <= sprite->draw.draw_end_x)
	{
		...
		if (render_s.u > 1.0)
			render_s.u = 1.0;
		render_s.tex_x = sprite->draw.origin_x + (int)(render_s.u
				* sprite->frame_w);
...
void	sprite_draw(t_game_data *data, t_sprite *sprite, t_infos infos)
{
	...
	if (!sprite || sprite->cam_z <= 0.1 || !sprite->sp_tex
		|| !sprite->sp_tex->addr)
		return ;
...
void	sprite_render_all(t_game_data *data, int start_x, int view_w,
		int view_h)
{
	t_sprite	**visible;
	int			i;
