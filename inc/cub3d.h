...
void			cleanup_i_d(t_cmp_data *cmp_data, char **already_checked,
...
int				comma_length_checker_helper(char *str, int *i, int *counter,
					int *comma);
...
int				is_valid_helper(t_file_data *file_data,
					t_helper_data *helper_data);
...
int				specific_store(t_file_data *file_data, char who_know,
					char *buffer);
...
int				store_r_p(char **to_be_splited, t_file_data *file_data,
					char *buffer);
...
void			render_3d_view(t_game_data *data, int start_x,
...
int				flip_text_horizontally(t_game_data *data,
					t_texture *current_tex);
...
void			set_door_data(t_game_data *data, t_door *door,
					double dist, double tex_u);
...
void			draw_minimap_door(t_game_data *data, t_minimap *minimap,
					t_door *door);
void			draw_tile(t_game_data *data, int *int_holder,
					t_minimap *minimap);
...
void			drawin_minidoors_helper(t_mini_draw *mini, t_game_data *data,
					t_minimap *minimap);
void			init_mini_draw(t_mini_draw *mini, t_minimap *minimap,
					t_door *door);
...
void			fill_outer_ppixel(t_game_data *data, int i, int j,
					int floor_color);
void			set_cercle_data(t_game_data *data, t_minimap *minimap,
					t_cercle *cercle);
